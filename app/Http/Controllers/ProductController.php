                ...
                if ($productable) {
                    if ($product->productable_type === 'App\\Models\\Land') {
                        ...
                        if ($productable->type === 'building' && $productable->partOfBuildings->isNotEmpty()) {
                            ...
                            $mediumFinance = $productable->buildingFinances->firstWhere('type_of_standing', 'medium');
                            ...
                            if ($mediumFinance) {
                                ...
                                $mountIncome = 0;
                                foreach ($productable->partOfBuildings as $part) {
                            ...
                            $productable->buildingFinances->each(function ($finance) {
                ...
                if ($product->proposedProducts->isNotEmpty()) {
                    $product->proposed_products = $product->proposedProducts->map(function ($proposedProduct) {
                        ...
                        if ($proposedProduct->productable_type === 'App\\Models\\Land') {
                            ...
                            if ($proposedProduct->productable->buildingFinances) {
                                $proposedProduct->productable->buildingFinances->each(function ($finance) {
                                    $finance->makeHidden(['created_at', 'updated_at']);
                                });
                            }
